...
class TkinterCustomButton(tkinter.Frame):
    ...
    def configure_color(self, bg_color=None, fg_color=None, hover_color=None,
                        text_color=None, image=None, image_hover=None):
        self.image_normal = image
        self.image_hover = image_hover
    ...
    def on_enter(self, event=0):
        ...
        if self.image_label is not None:
            # change background color of image_label
            if self.image_hover is None:
                self.image_label.configure(bg=self.hover_color)
            else:
                self.image_label.configure(image=self.image_hover)
    ...
    def on_leave(self, event=0):
        ...
        if self.image_label is not None:
            ...
            if self.image_hover is None:
                ...
                self.image_label.configure(image=self.image_normal)
...
class HandCustomButton(TkinterCustomButton):
    def __init__(self, master, image, image_hover, command):
    ...
    def custom_configure(self, image, image_hover, command):
        super().configure_color(image=image, image_hover=image_hover)
        super().configure_command(command)
        self.draw()
